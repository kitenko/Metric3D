    ...
    def __init__(self, inp_ch=32, output_chal=1, down_sample_times=3, channel_mode="v0"):
        ...
        for i in range(down_sample_times):
            setattr(
                ...
                "conv_%d" % i,
                nn.Sequential(
                    basic_block(channels[i], channels[i + 1], stride=2), basic_block(channels[i + 1], channels[i + 1])
                ),
            )
        for i in range(down_sample_times - 1, -1, -1):
            setattr(
                ...
                nn.Sequential(
                    nn.ConvTranspose2d(
                        channels[i + 1], channels[i], kernel_size=3, padding=1, output_padding=1, stride=2, bias=False
                    ),

...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[], out_dims=2, tuning_mode=None):
        ...
        self.gru08 = ConvGRU(
            hidden_dims[2], encoder_output_dim + hidden_dims[1] * (self.n_gru_layers > 1), tuning_mode=tuning_mode
        )
        self.gru16 = ConvGRU(
            hidden_dims[1], hidden_dims[0] * (self.n_gru_layers == 3) + hidden_dims[2], tuning_mode=tuning_mode
        )
...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, planes, norm_fn="group", stride=1, tuning_mode=None):
        ...
        self.conv1 = Conv2dLoRA(
            in_planes, planes, kernel_size=3, padding=1, stride=stride, r=8 if tuning_mode == "lora" else 0
        )
        ...
        if stride == 1 and in_planes == planes:
            ...
            self.downsample = nn.Sequential(
                Conv2dLoRA(in_planes, planes, kernel_size=1, stride=stride, r=8 if tuning_mode == "lora" else 0),
                self.norm3,
            )
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, channels, tuning_mode=None):
        ...
        self.conv1 = Conv2dLoRA(
            channels, channels, kernel_size=3, stride=1, padding=1, r=8 if tuning_mode == "lora" else 0
        )
...
class FuseBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, fuse=True, upsample=True, scale_factor=2, tuning_mode=None):
        ...
        self.out_conv = Conv2dLoRA(
            in_channels, out_channels, kernel_size=1, stride=1, padding=0, r=8 if tuning_mode == "lora" else 0
        )
    ...
    def __init__(self, in_features, use_cls_token=True, num_register_tokens=0, tuning_mode=None):
        ...
        if self.use_cls_token == True:
            ...
            self.project_learn = LoRALinear(
                (1 + num_register_tokens) * in_features, in_features, bias=False, r=8 if tuning_mode == "lora" else 0
            )
            self.act = nn.GELU()
        else:
...
class Token2Feature(nn.Module):
    # From DPT
    def __init__(
        self, vit_channel, feature_channel, scale_factor, use_cls_token=True, num_register_tokens=0, tuning_mode=None
    ):
...
class DecoderFeature(nn.Module):
    def __init__(self, vit_channel, num_ch_dec=[128, 256, 512, 1024, 1024], use_cls_token=True, tuning_mode=None):
        ...
        self.upconv_3 = FuseBlock(
            self.num_ch_dec[4], self.num_ch_dec[3], fuse=False, upsample=False, tuning_mode=tuning_mode
        )

        self.upconv_2 = FuseBlock(self.num_ch_dec[3], self.num_ch_dec[2], tuning_mode=tuning_mode)

        self.upconv_1 = FuseBlock(
            self.num_ch_dec[2], self.num_ch_dec[1] + 2, scale_factor=7 / 4, tuning_mode=tuning_mode
        )
        ...
        for itr in range(self.iters):
            ...
            if self.n_gru_layers >= 2 and self.slow_fast_gru:  # Update low-res GRU and mid-res GRU
                net_list = self.update_block(
                    net_list, inp_list, iter32=self.n_gru_layers == 3, iter16=True, iter08=False, update=False
                )
            net_list, up_mask, delta_flow = self.update_block(
                net_list, inp_list, None, flow, iter32=self.n_gru_layers == 3, iter16=self.n_gru_layers >= 2
            )

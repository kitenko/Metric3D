...
from mono.utils.visualization import create_html, save_normal_val_imgs, save_raw_imgs, save_val_imgs
...
def postprocess_per_image(
    ...
    pred_depth = torch.nn.functional.interpolate(
        pred_depth[None, None, :, :], [rgb_origin.shape[0], rgb_origin.shape[1]], mode="bilinear"
    ...
    if gt_depth is not None:
        ...
        pred_depth = torch.nn.functional.interpolate(
            pred_depth[None, None, :, :], (gt_depth.shape[0], gt_depth.shape[1]), mode="bilinear"
        ).squeeze()  # to original size
    ...
    if normal_out is not None:
        ...
        pred_normal = torch.nn.functional.interpolate(
            pred_normal[None, :], size=[rgb_origin.shape[0], rgb_origin.shape[1]], mode="bilinear", align_corners=True
        ).squeeze()
        ...
        if False:
            pred_normal = torch.nn.functional.interpolate(
                pred_normal, size=gt_normal.shape[2:], mode="bilinear", align_corners=True
            )

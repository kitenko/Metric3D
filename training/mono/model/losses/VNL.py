...
class VNLoss(nn.Module):
    ...
    def __init__(
        ...
        sample_ratio=0.15,
        loss_weight=1.0,
        data_type=["sfm", "stereo", "lidar", "denselidar", "denselidar_nometric", "denselidar_syn"],
        **kwargs,
    ):
    ...
    def filter_mask(self, p123, gt_xyz, delta_cos=0.867, delta_diff_x=0.005, delta_diff_y=0.005, delta_diff_z=0.005):
        ...
        ###ignore linear
        pw_diff = torch.cat(
            [pw12[:, :, :, np.newaxis], pw13[:, :, :, np.newaxis], pw23[:, :, :, np.newaxis]], 3
        )  # [b, n, 3, 3]
        ...
        nm = torch.bmm(
            q_norm.contiguous().view(m_batchsize * groups, index, 1), q_norm.view(m_batchsize * groups, 1, index)
        )  # []
    ...
    def select_points_groups(self, gt_depth, pred_depth, intrinsic, mask):
        ...
        mask, pw_groups_gt = self.filter_mask(
            p123, pw_gt, delta_cos=0.867, delta_diff_x=0.005, delta_diff_y=0.005, delta_diff_z=0.005
        )

...
class ConfidenceGuideLoss(nn.Module):
    ...
    def __init__(
        self, loss_weight=1, data_type=["stereo", "lidar", "denselidar"], loss_gamma=0.9, conf_loss=True, **kwargs
    ):
    ...
    def forward(self, samples_pred_list, target, coord_list, mask=None, **kwargs):
        ...
        for i, (pred, coord) in enumerate(zip(samples_pred_list, coord_list)):
            ...
            if self.conf_loss:
                ...
                conf_loss = torch.sum(torch.abs(pred_conf - gt_confidence) * conf_mask) / (

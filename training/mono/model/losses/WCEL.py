...
class WCELoss(nn.Module):
    ...
    def config_bins(self):
        # Modify some configs
        self.depth_bins_interval = (torch.log10(torch.tensor(self.depth_max)) - self.depth_min_log) / self.bins_num
        bins_edges_in_log = self.depth_min_log + self.depth_bins_interval * torch.tensor(
        ...
        bins_weight = torch.tensor(
            [[np.exp(-self.alpha * (i - j) ** 2) for i in range(self.bins_num)] for j in np.arange(self.bins_num)]
        ).cuda()
        ...
        for i in range(h_blocks):
            for j in range(w_blocks):
                ...
                bins_ij = _depth_to_bins_block_(
                    depth[:, :, h_start:h_end, w_start:w_end], mask[:, :, h_start:h_end, w_start:w_end], depth_edges
                )

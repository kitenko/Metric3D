...
class GRUSequenceLoss(nn.Module):
    ...
    def forward(self, predictions_list, target, stereo_depth, confidence_list=None, mask=None, **kwargs):
        ...
        self.batch_with_stereo = torch.tensor(
            [1 if batch_dataset in self.stereo_dataset else 0 for batch_dataset in batches_dataset], device=device
        )[:, None, None, None]
        ...
        for i, prediction in enumerate(predictions_list):
            ...
            mask_stereo = 1 + torch.nn.functional.max_pool2d(
                -torch.nn.functional.max_pool2d(mask * 1.0, 3, stride=1, padding=1, dilation=1),
                3,
                stride=1,
                padding=1,
                dilation=1,
            ...
            stereo_depth_loss = torch.sum(self.batch_with_stereo * stereo_diff * mask_stereo) / (

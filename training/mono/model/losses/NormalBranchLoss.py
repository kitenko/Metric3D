...
class NormalBranchLoss(nn.Module):
    ...
    def forward_R(self, norm_out, gt_norm, gt_norm_mask):
        ...
        if self.loss_type == "L1":
            ...
            valid_mask = (
                gt_norm_mask[:, 0, :, :].float() * (dot.detach() < 0.999).float() * (dot.detach() > -0.999).float()
            )
            ...
            loss_pixelwise = -torch.log(kappa) - (kappa * (dot - 1)) + torch.log(1 - torch.exp(-2 * kappa))
            ...
            loss_pixelwise = (
                -torch.log(torch.square(kappa) + 1) + kappa * torch.acos(dot) + torch.log(1 + torch.exp(-kappa * np.pi))
            )
            loss = torch.mean(loss_pixelwise)
...
class DeNoConsistencyLoss(nn.Module):
    def __init__(
        self,
        loss_weight=1.0,
        data_type=["stereo", "lidar", "denselidar", "denselidar_nometric", "denselidar_syn"],
        loss_fn="NLL_ours",
        sky_id=142,
        scale=1,
    ...
    def forward(self, **kwargs):
        ...
        self.batch_enabled = torch.tensor(
            [1 if batch_dataset not in self.disable_dataset else 0 for batch_dataset in batches_dataset],
            device=device,
            dtype=torch.bool,
        )[:, None, None, None]
        ...
        self.batch_with_norm_sky = torch.tensor(
            [1 if batch_dataset not in self.no_sky_dataset else 0 for batch_dataset in batches_dataset],
            device=device,
            dtype=torch.bool,
        )[:, None, None, None]
    ...
    def forward_J(self, prediction, confidence, normal_out_list, intrinsic, pad_mask, sem_mask=None, **kwargs):
        ...
        normal, new_mask = self.depth2normal(
            prediction.detach() if self.depth_detach else prediction, intrinsic, pad_mask
        )
        # mask sky
    ...
    def forward_S(
        ...
        **kwargs,
        ...
        # scale = 1.0
        normal, new_mask = self.depth2normal(
            prediction.detach() if self.depth_detach else prediction, intrinsic, pad_mask, scale
        )
        ...
        # all mask
        normal_mask = (
            ~torch.all(normal == 0, dim=1, keepdim=True) & new_mask & sample_mask_n & sample_mask_d & conf_mask
        )
        if normal_mask.sum() < 10:
            return 0 * prediction_normal.sum()
        ...
        for i, (norm, conf, depth) in enumerate(zip(normal_out_list, confidence_list, predictions_list)):
            ...
            if i == 0:
                ...
                curr_loss = self.forward_S(
                    ...
                    new_intrinsic,
                    torch.nn.functional.interpolate(pad_mask.float(), scale_factor=0.25).bool(),
                ...
                is_initial_pair = False
                curr_loss = self.forward_S(
                    depth, conf, intrinsic, pad_mask, norm, sem_mask, target, is_initial_pair, scale=kwargs["scale"]
                )

...
class HDNRandomLoss(nn.Module):
    ...
    def get_random_masks_for_batch(self, depth_gt: torch.Tensor, mask_valid: torch.Tensor) -> torch.Tensor:
        ...
        mask_new = [
            (depth_gt >= sample_min_d[i]) & (depth_gt < sample_max_d[i] + 1e-30) & mask_valid
    ...
    def forward(self, prediction, target, mask=None, **kwargs):
        ...
        self.batch_valid = torch.tensor(
            [1 if batch_dataset not in self.disable_dataset else 0 for batch_dataset in batches_dataset], device=device
        )[:, None, None, None]

...
class PWNPlanesLoss(nn.Module):
    ...
    def init_image_coor(self, B, H, W):
        u = (
            torch.arange(0, H, dtype=torch.float32, device="cuda").contiguous().view(1, H, 1).expand(1, H, W)
        )  # [1, H, W]
        v = (
            torch.arange(0, W, dtype=torch.float32, device="cuda").contiguous().view(1, 1, W).expand(1, H, W)
        )  # [1, H, W]
    ...
    def filter_mask(self, pw_pred):
        ...
        # Ignore linear
        xy_diff = torch.cat(
            [xy12[:, :, :, np.newaxis], xy13[:, :, :, np.newaxis], xy23[:, :, :, np.newaxis]], 3
        )  # [b, n, 2(xy), 3]
        m_batchsize, groups, coords, index = xy_diff.shape
        proj_query = (
            xy_diff.contiguous().view(m_batchsize * groups, -1, index).permute(0, 2, 1).contiguous()
        )  # [bn, 3(p123), 2(xy)]

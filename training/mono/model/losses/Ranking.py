...
class RankingLoss(nn.Module):
    ...
    def forward(self, prediction, target, mask=None, **kwargs):
        ...
        for i in range(n):
            ...
            inputs_A, inputs_B, targets_A, targets_B, consistent_masks_A, consistent_masks_B = randomSampling(
                prediction[i, :], target[i, :], mask[i, :], self.mask_value, self.point_pairs
            )
    ...
    def get_loss(self, prediction, target, mask=None, input=None, **kwargs):
        ...
        for i in range(n):
            ...
            inputs_A, inputs_B, targets_A, targets_B, masks_A, masks_B, sample_num = edgeGuidedSampling(
                prediction[i, :], target[i, :], edges_img[i], thetas_img[i], mask[i, :], h, w
            )
            ...
            random_inputs_A, random_inputs_B, random_targets_A, random_targets_B, random_masks_A, random_masks_B = (
                randomSampling(prediction[i, :], target[i, :], mask[i, :], self.mask_value, random_sample_num)
            )
            ...
            unequal_loss = torch.log(1 + torch.exp((-inputs_A_merge + inputs_B_merge) * labels))[

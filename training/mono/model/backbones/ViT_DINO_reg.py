    ...
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = drop_add_residual_stochastic_depth(
                x, residual_func=attn_residual_func, sample_drop_ratio=self.sample_drop_ratio, attn_bias=attn_bias
            )
            x = drop_add_residual_stochastic_depth(
                x,
...
def drop_add_residual_stochastic_depth(
    x: Tensor, residual_func: Callable[[Tensor], Tensor], sample_drop_ratio: float = 0.0, attn_bias=None
) -> Tensor:
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, masks=None):
        ...
        if self.multi_output == False:
            ...
            return [
                features,
                (B, (H + pad_h) // self.patch_size, (W + pad_w) // self.patch_size, H, W, self.num_register_tokens),
            ]
        else:
            features = []
            for blk in self.blocks:

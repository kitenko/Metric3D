    ...
    def __init__(self, in_dim, output_dim):
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(in_dim[0], dim[0], "layer", stride=1), nn.Conv2d(dim[0], dim[0], 3, padding=1)
            )
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(in_dim[1], dim[1], "layer", stride=1), nn.Conv2d(dim[1], dim[1], 3, padding=1)
            )
            output_list.append(conv_out)
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(in_dim[2], dim[2], "layer", stride=1), nn.Conv2d(dim[2], dim[2], 3, padding=1)
            )
            output_list.append(conv_out)
...
class Token2Feature(nn.Module):
    ...
    def __init__(self, vit_channel, feature_channel, scale_factor, use_cls_token=True, num_register_tokens=0):
        ...
        self.readoper = Readout(
            in_features=vit_channel, use_cls_token=use_cls_token, num_register_tokens=num_register_tokens
        )
...
class RAFTDepthNormalDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        self.token2feature = EncoderFeature(
            self.in_channels[0], self.feature_channels, self.use_cls_token, self.num_register_tokens
        )
        ...
        self.context_zqr_convs = nn.ModuleList(
            [
                nn.Conv2d(self.context_dims[i], self.hidden_dims[i] * 3, 3, padding=3 // 2)

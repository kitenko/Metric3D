...
from mono.datasets.distributed_sampler import build_data_array, build_dataset_n_sampler_with_cfg
from mono.model.criterion import build_criterions
from mono.model.monodepth_model import get_configured_monodepth_model
from mono.utils.avg_meter import MetricAverageMeter
from mono.utils.comm import TrainingStats, get_rank, main_process, reduce_dict
...
def do_train(local_rank: int, cfg: dict):
    ...
    if cfg.distributed:
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            model.cuda(), device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False
        )
    else:
    ...
    if cfg.load_from and cfg.resume_from is None:
        model, _, _, loss_scaler = load_ckpt(
            cfg.load_from, model, optimizer=None, scheduler=None, strict_match=False, loss_scaler=loss_scaler
        )
...
def train_by_iters(cfg, model, optimizer, lr_scheduler, train_dataloader, val_dataloader):
    ...
    try:
        ...
        while step < max_iters:
            ...
            # validate the model
            if cfg.evaluation.online_eval and (step + 1) % eval_interval == 0 and val_dataloader is not None:
                ...
                if (
                    ...
                    if isinstance(val_dataloader, list):
                        val_err = validate_multiple_dataset(
                            cfg, ((step + acc_batch) // acc_batch), model, val_dataloader, tb_logger
                        )
                    else:

...
class Compose(object):
    ...
    def __call__(
        self, images, labels, intrinsics, cam_models=None, normals=None, other_labels=None, transform_paras=None
    ):
        for t in self.transforms:
            images, labels, intrinsics, cam_models, normals, other_labels, transform_paras = t(
                images, labels, intrinsics, cam_models, normals, other_labels, transform_paras
            )
    ...
    def __call__(
        ...
        if cam_models is not None:
            for i, cam_model in enumerate(cam_models):
                cam_models[i] = (
                    torch.from_numpy(cam_model.transpose((2, 0, 1))).float() if cam_model is not None else None
                )
        ...
        if other_labels != None:
            ...
            for i, other_label_i in enumerate(other_labels):
                other_labels[i] = cv2.resize(
                    other_label_i, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_NEAREST
                )
        ...
        if normals is not None:
            for i, normal in enumerate(normals):
                ...
                normals[i] = cv2.copyMakeBorder(
                    normal, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=0
                )
...
class RandomCrop(object):
    ...
    def main_data_transform(
        self, image, label, intrinsic, cam_model, pad_h, pad_w, pad_h_half, pad_w_half, h_off, w_off
    ):
    ...
    def __call__(
        ...
        for i in range(len(images)):
            ...
            cam_model = cam_models[i] if cam_models is not None and i < len(cam_models) else None
            img, label, intrinsic, cam_model = self.main_data_transform(
                img, label, intrinsic, cam_model, pad_h, pad_w, pad_h_half, pad_w_half, h_off, w_off
            )
            images[i] = img
            if label is not None:
                labels[i] = label
...
class AdjustSize(object):
    ...
    def __call__(
        ...
        for i in range(len(images)):
            ...
            intrinsic = intrinsics[i] if i < len(intrinsics) else None
            cam_model = cam_models[i] if cam_models is not None and i < len(cam_models) else None
            img, label, intrinsic, cam_model, pad = self.main_data_transform(img, label, intrinsic, cam_model)
            images[i] = img
            if label is not None:
                labels[i] = label
            if intrinsic is not None:
...
class RandomHorizontalFlip(object):
    ...
    def __call__(
        ...
        for i in range(len(images)):
            ...
            intrinsic = intrinsics[i] if i < len(intrinsics) else None
            cam_model = cam_models[i] if cam_models is not None and i < len(cam_models) else None
            img, label, intrinsic, cam_model = self.main_data_transform(img, label, intrinsic, cam_model, rotate)
            images[i] = img
            if label is not None:
                labels[i] = label
            if intrinsic is not None:
...
class RandomBlur(object):
    def __init__(self, aver_kernal=(2, 10), motion_kernal=(5, 15), angle=[-80, 80], prob=0.3, **kwargs):

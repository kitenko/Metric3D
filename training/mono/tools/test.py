...
import numpy as np
from mono.datasets.distributed_sampler import build_dataset_n_sampler_with_cfg, log_canonical_transfer_info
...
def main(args):
    ...
    if args.show_dir is not None:
        ...
        cfg.show_dir = osp.join("./show_dirs", osp.splitext(osp.basename(args.config))[0], args.timestamp)
...
def main_worker(local_rank: int, cfg: dict, launcher: str):
    ...
    test_dataloader = torch.utils.data.DataLoader(
        dataset=test_dataset, batch_size=1, num_workers=1, sampler=test_sampler, drop_last=False
    )

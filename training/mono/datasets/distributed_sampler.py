    ...
    def __iter__(self):
        ...
        self.logger.info(
            "Sample list of {} in rank {} is: {}".format(
                self.phase, self.global_rank, " ".join(map(str, self.sample_indices_array[-20:-10]))
            )
        )
    ...
    def create_samplers(self):
        ...
        for gi, dataset_group in enumerate(self.dataset_merge.datasets):
            ...
            for di, dataset_i in enumerate(dataset_group.datasets):
                ...
                indices_expand_list = (
                    np.array(indices_expand_list) + dataset_cumulative_sizes[gi] + group_cumulative_sizes[di]
                )
...
def build_data_array(cfg, phase):
    ...
    for group_i in dataname_array:
        ...
        for data_i in group_i:
            ...
            dataset_i = obj_cls(args[data_i_cfg_name], phase, db_info=data_i_db_info, **cfg.data_basic)

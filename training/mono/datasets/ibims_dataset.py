...
class IBIMSDataset(BaseDataset):
    def __init__(self, cfg, phase, **kwargs):
        ...
        self.avg = torch.nn.AvgPool2d(
            kernel_size=7, stride=1, ceil_mode=False, count_include_pad=True, divisor_override=None
        )
    ...
    def load_batch(self, meta_data, data_path):
        ...
        curr_normal = self.load_norm_label(
            data_path["normal_path"], H=curr_rgb.shape[0], W=curr_rgb.shape[1], depth=curr_depth, K=curr_intrinsic
    ...
    def load_norm_label(self, norm_path, H, W, depth, K):
        ...
        y, x = torch.meshgrid(
            [torch.arange(0, 480, dtype=torch.float32), torch.arange(0, 640, dtype=torch.float32)], indexing="ij"
        )
